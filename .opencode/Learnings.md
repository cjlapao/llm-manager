...
### Command Registry + Typed YAML Parameters (2026-04-19)

14. **Factory closures for `init()`-based command registration** — `internal/cmd/registry.go` defines a `Command` interface, `CommandFactory func(root *RootCommand) Command` type, and `CommandDispatcher`. Each command registers itself via `init()` using a factory closure rather than a pre-constructed instance. **Never Again**: When registering commands in `init()`, never pass pre-constructed instances to a registry if the constructors need non-nil dependencies (like `db` or `cfg`). Use factory closures to defer construction until the dependencies are available, avoiding nil pointer dereferences during package initialization.

15. **Type preservation through JSON serialization** — `pkg/yamlparser/typed.go` introduces `TypedValue` (preserves bool/int/float/string), `InferType()` for automatic type inference from strings, `ParseTypedCommandArgs()` for bulk conversion, and `CommandArgsToJSON()` for type-preserving JSON serialization to DB. Typed values serialize to JSON preserving native types (bool→bool, int→number, string→string), so the database stores properly typed data rather than all-strings. Existing YAML files with string command args still work — typed parsing is applied at import time.
